...
All resources follow the `A3-G7-` prefix for consistent identification and management.

| Resource Type | AWS Console Name | Purpose |
|---|---|---|
| **Network Foundation** |
| VPC | A3-G7-VPC | Primary virtual network |
| Internet Gateway | A3-G7-IGW | Outbound internet access |
| NAT Gateway | A3-G7-NAT | Private subnet internet access |
| **Subnets** |
| Public Subnet AZ1 | A3-G7-Public-Subnet-AZ1 | ALB and NAT (us-east-1a) |
| Public Subnet AZ2 | A3-G7-Public-Subnet-AZ2 | ALB redundancy (us-east-1b) |
| Private EC2 Subnet AZ1 | A3-G7-Private-EC2-Subnet-AZ1 | Web servers (us-east-1a) |
| Private EC2 Subnet AZ2 | A3-G7-Private-EC2-Subnet-AZ2 | Web servers (us-east-1b) |
| Private RDS Subnet AZ1 | A3-G7-Private-RDS-Subnet-AZ1 | Database primary (us-east-1a) |
| Private RDS Subnet AZ2 | A3-G7-Private-RDS-Subnet-AZ2 | Database standby (us-east-1b) |
| **Routing** |
| Public Route Table | A3-G7-Public-RT | Routes to Internet Gateway |
| Private Route Table | A3-G7-Private-RT | Routes to NAT Gateway |
| **Security** |
| ALB Security Group | A3-G7-ALB-SG | Load balancer firewall |
| EC2 Security Group | A3-G7-EC2-SG | Web server firewall |
| RDS Security Group | A3-G7-RDS-SG | Database firewall |
| IAM Role | A3-G7-SSM-Role | EC2 parameter access |
| **Compute** |
| EC2 Instance AZ1 | A3-G7-EC2-AZ1 | Initial web server |
| EC2 Instance AZ2 | A3-G7-EC2-AZ2 | ASG-launched instances |
| AMI | A3-G7-AMI | Golden image |
| Launch Template | A3-G7-LT | ASG configuration |
| **Database** |
| RDS Instance | A3-G7-RDS | MySQL database |
| DB Subnet Group | A3-G7-DB-Subnet-Group | RDS network placement |
| **Load Balancing & Scaling** |
| Target Group | A3-G7-TG | EC2 health monitoring |
| Application Load Balancer | A3-G7-ALB | Traffic distribution |
| Auto Scaling Group | A3-G7-ASG | Dynamic scaling |
...
| **Public Subnets** |
| A3-G7-Public-Subnet-AZ1 | 10.0.1.0/24 | 251 | ALB, NAT (us-east-1a) |
| A3-G7-Public-Subnet-AZ2 | 10.0.2.0/24 | 251 | ALB redundancy (us-east-1b) |
| **Private EC2 Subnets** |
| A3-G7-Private-EC2-Subnet-AZ1 | 10.0.3.0/24 | 251 | Web servers (us-east-1a) |
| A3-G7-Private-EC2-Subnet-AZ2 | 10.0.4.0/24 | 251 | Web servers (us-east-1b) |
| **Private RDS Subnets** |
| A3-G7-Private-RDS-Subnet-AZ1 | 10.0.5.0/24 | 251 | Database (us-east-1a) |
| A3-G7-Private-RDS-Subnet-AZ2 | 10.0.6.0/24 | 251 | Database (us-east-1b) |
...
| Name tag | `A3-G7-VPC` | Exactly as shown |
...
**Verification:**
- VPC list shows A3-G7-VPC with State = Available
- CIDR shows 10.0.0.0/16

📸 **Screenshot Required:** VPC dashboard showing A3-G7-VPC with CIDR 10.0.0.0/16
...
| Field | Value |
|---|---|
| Available VPCs | Select `A3-G7-VPC` from dropdown |
...
- VPC ID column shows A3-G7-VPC

📸 **Screenshot Required:** Internet Gateway showing State = Attached to A3-G7-VPC
...
| Field | Value |
|---|---|
| VPC ID | Select `A3-G7-VPC` from dropdown |
...
| Field | Value |
|---|---|
| Subnet name | `A3-G7-Public-Subnet-AZ1` |
...
| Field | Value |
|---|---|
| Subnet name | `A3-G7-Public-Subnet-AZ2` |
...
| Field | Value |
|---|---|
| Subnet name | `A3-G7-Private-EC2-Subnet-AZ1` |
| Availability Zone | `us-east-1a` |
...
| Field | Value |
|---|---|
| Subnet name | `A3-G7-Private-EC2-Subnet-AZ2` |
| Availability Zone | `us-east-1b` |
...
| Field | Value |
|---|---|
| Subnet name | `A3-G7-Private-RDS-Subnet-AZ1` |
| Availability Zone | `us-east-1a` |
...
| Field | Value |
|---|---|
| Subnet name | `A3-G7-Private-RDS-Subnet-AZ2` |
| Availability Zone | `us-east-1b` |
...
**For A3-G7-Public-Subnet-AZ1:**
1. In subnet list, find and **select** A3-G7-Public-Subnet-AZ1 (checkbox on left)
2. Click **Actions** → **Edit subnet settings**
3. Under "Auto-assign IP settings":
   - ✅ Check **Enable auto-assign public IPv4 address**
4. Click **Save**

**For A3-G7-Public-Subnet-AZ2:**
1. Select A3-G7-Public-Subnet-AZ2
2. Click **Actions** → **Edit subnet settings**
...
|---|---|---|---|
| A3-G7-Public-Subnet-AZ1 | us-east-1a | 10.0.1.0/24 | ✅ Yes |
| A3-G7-Public-Subnet-AZ2 | us-east-1b | 10.0.2.0/24 | ✅ Yes |
| A3-G7-Private-EC2-Subnet-AZ1 | us-east-1a | 10.0.3.0/24 | ❌ No |
| A3-G7-Private-EC2-Subnet-AZ2 | us-east-1b | 10.0.4.0/24 | ❌ No |
| A3-G7-Private-RDS-Subnet-AZ1 | us-east-1a | 10.0.5.0/24 | ❌ No |
| A3-G7-Private-RDS-Subnet-AZ2 | us-east-1b | 10.0.6.0/24 | ❌ No |
...
| Field | Value | Notes |
|---|---|---|
| Name | `A3-G7-NAT` | Optional but recommended |
| Subnet | `A3-G7-Public-Subnet-AZ1` | **MUST be a public subnet** |
...
**Verification:**
- State = Available
- Subnet = A3-G7-Public-Subnet-AZ1 (10.0.1.0/24)
...
| Field | Value |
|---|---|
| Name | `A3-G7-Public-RT` |
| VPC | Select `A3-G7-VPC` from dropdown |
...
The newly created route table is automatically displayed. If not, click on A3-G7-Public-RT.
...
| Destination | Target |
|---|---|
| `0.0.0.0/0` | Select **Internet Gateway**, then select `A3-G7-IGW` |
...
12. Select (checkbox) both:
    - ✅ A3-G7-Public-Subnet-AZ1
    - ✅ A3-G7-Public-Subnet-AZ2
13. Click **Save associations**

**Verification:**
- Routes tab shows:
  - 10.0.0.0/16 → local
  - 0.0.0.0/0 → igw-xxxxx (A3-G7-IGW)
- Subnet associations shows 2 subnets associated

📸 **Screenshot Required:** A3-G7-Public-RT showing routes and subnet associations
...
| Field | Value |
|---|---|
| Name | `A3-G7-Private-RT` |
| VPC | Select `A3-G7-VPC` |
...
| Destination | Target |
|---|---|
| `0.0.0.0/0` | Select **NAT Gateway**, then select `A3-G7-NAT` |
...
9. Select (checkbox) both:
    - ✅ A3-G7-Private-EC2-Subnet-AZ1
    - ✅ A3-G7-Private-EC2-Subnet-AZ2
10. **DO NOT select** A3-G7-Private-RDS-Subnet-AZ1 or A3-G7-Private-RDS-Subnet-AZ2
11. Click **Save associations**

**Verification:**
- Routes tab shows:
  - 10.0.0.0/16 → local
  - 0.0.0.0/0 → nat-xxxxx (A3-G7-NAT)
...
📸 **Screenshot Required:** A3-G7-Private-RT showing NAT Gateway route and EC2 subnet associations
...
```
Internet → A3-G7-ALB-SG → A3-G7-EC2-SG → A3-G7-RDS-SG
```
...
| Field | Value |
|---|---|
| Security group name | `A3-G7-ALB-SG` |
...
📸 **Screenshot Required:** A3-G7-ALB-SG inbound rules showing HTTP from 0.0.0.0/0
...
| Field | Value |
|---|---|
| Security group name | `A3-G7-EC2-SG` |
...
- In the text field that appears, type `A3-G7-ALB-SG`
- Select the security group that appears: "A3-G7-ALB-SG (sg-xxxxx)"
...
📸 **Screenshot Required:** A3-G7-EC2-SG inbound rules showing:
- HTTP from A3-G7-ALB-SG
- SSH from your IP
...
| Field | Value |
|---|---|
| Security group name | `A3-G7-RDS-SG` |
...
- In text field, type `A3-G7-EC2-SG`
- Select "A3-G7-EC2-SG (sg-xxxxx)" from dropdown
...
|---|---|
| A3-G7-ALB-SG | HTTP (80) from 0.0.0.0/0 |
| A3-G7-EC2-SG | HTTP (80) from A3-G7-ALB-SG, SSH (22) from My IP |
| A3-G7-RDS-SG | MySQL (3306) from A3-G7-EC2-SG |

📸 **Screenshot Required:** A3-G7-RDS-SG inbound rules showing MySQL from A3-G7-EC2-SG

✅ **TASK 5 COMPLETE**

---

### TASK 6: Create IAM Role for EC2

This role allows EC2 instances to read from Systems Manager Parameter Store without embedding credentials in code.
...
| Use case - Service | Select **EC2** |
| Use case - Use case | EC2 (already selected) |

4. Click **Next**

**Step 2 - Add permissions:**
5. In search box, type: `SSMReadOnly`
6. Find and check ✅ **AmazonSSMReadOnlyAccess**
7. Click **Next**

**Step 3 - Name, review, and create:**
| Field | Value |
|---|---|
| Role name | `A3-G7-SSM-Role` |
| Description | `Allows EC2 to read from Systems Manager Parameter Store` |

8. Scroll down and click **Create role**

**Verification:**
- Role list shows A3-G7-SSM-Role
- Click on the role name
- "Permissions" tab shows policy: AmazonSSMReadOnlyAccess
- "Trust relationships" tab shows Service: ec2.amazonaws.com

📸 **Screenshot Required:** A3-G7-SSM-Role showing AmazonSSMReadOnlyAccess policy attached
...
- [ ] A3-G7-VPC created (10.0.0.0/16)
- [ ] A3-G7-IGW attached to A3-G7-VPC
- [ ] All 6 subnets created with correct names, AZs, and CIDRs
- [ ] Public subnets have auto-assign public IPv4 enabled
- [ ] A3-G7-NAT in A3-G7-Public-Subnet-AZ1, State = Available
- [ ] A3-G7-Public-RT routes 0.0.0.0/0 to IGW
- [ ] A3-G7-Public-RT associated with both public subnets
- [ ] A3-G7-Private-RT routes 0.0.0.0/0 to NAT
- [ ] A3-G7-Private-RT associated with both EC2 private subnets only
- [ ] A3-G7-ALB-SG allows HTTP from 0.0.0.0/0
- [ ] A3-G7-EC2-SG allows HTTP from A3-G7-ALB-SG and SSH from My IP
- [ ] A3-G7-RDS-SG allows MySQL from A3-G7-EC2-SG
- [ ] A3-G7-SSM-Role created with AmazonSSMReadOnlyAccess
- [ ] All required screenshots captured

### Screenshots Checklist for Member 3:
1. ✅ A3-G7-VPC dashboard with CIDR
2. ✅ A3-G7-IGW showing State = Attached
3. ✅ All 6 subnets with names and CIDRs
4. ✅ A3-G7-NAT showing Available state and Elastic IP
5. ✅ A3-G7-Public-RT routes and subnet associations
6. ✅ A3-G7-Private-RT routes and subnet associations
7. ✅ A3-G7-ALB-SG inbound rules
8. ✅ A3-G7-EC2-SG inbound rules
9. ✅ A3-G7-RDS-SG inbound rules
10. ✅ A3-G7-SSM-Role with policy
...
|---|---|
| VPC | Select `A3-G7-VPC` |
| Subnet | Select `A3-G7-Private-EC2-Subnet-AZ1` |
| Auto-assign public IP | **Disable** |
| Firewall (security groups) | Select existing security group |
| Common security groups | Select `A3-G7-EC2-SG` |
...
| Field | Value |
|---|---|
| IAM instance profile | Select `A3-G7-SSM-Role` |
...
- Instance type: t2.small
- Subnet: A3-G7-Private-EC2-Subnet-AZ1
- Security group: A3-G7-EC2-SG
...
**Start session:**
4. You should see A3-G7-EC2-AZ1 in the list of available instances
   - If you don't see it, wait 1-2 minutes and refresh - the SSM agent needs time to register
5. Select the radio button next to **A3-G7-EC2-AZ1**
...
sudo yum update -y
```
This will take 1-2 minutes. You'll see packages being downloaded and installed.

**Step 3.2 - Install Apache, PHP, and MySQL client:**
```bash
sudo yum install -y httpd php php-mysqlnd
```

Expected output: "Complete!" after installation.

**Step 3.3 - Start Apache web server:**
```bash
...
**Step 3.4 - Enable Apache to start on boot:**
```bash
...
**Step 3.5 - Verify Apache is running:**
```bash
...
**Step 3.6 - Test Apache locally:**
```bash
curl http://localhost
```

Expected output: HTML code of the default Apache test page.

**Verification:**
- httpd service is "active (running)"
- curl localhost returns HTML

📸 **Screenshot Required:** Terminal showing `sudo systemctl status httpd` with "active (running)" status
...
You have a website.zip file containing the PHP application. Since the EC2 instance has no public IP, you'll use Session Manager's file transfer feature.

#### Method A: Upload via Session Manager (Recommended)

**Step 4.1 - Upload file:**
1. In Session Manager terminal window, look for the **Download / Upload** button (top right, looks like ⬇️⬆️)
2. Click **Upload file**
3. Browse to your `website.zip` file
4. Click **Open**
5. Wait for "File uploaded successfully" message
6. File is uploaded to: `/home/ssm-user/website.zip`

**Step 4.2 - Move file to ec2-user home:**
```bash
sudo mv /home/ssm-user/website.zip /home/ec2-user/
sudo chown ec2-user:ec2-user /home/ec2-user/website.zip
```

**Step 4.3 - Extract to web directory:**
```bash
cd /home/ec2-user
sudo unzip website.zip -d /var/www/html/
```

Expected output: List of files being extracted (index.php, images/, etc.)

**Step 4.4 - Set correct permissions:**
```bash
sudo chown -R apache:apache /var/www/html/
sudo chmod -R 755 /var/www/html/
```

**Step 4.5 - Verify files are in place:**
```bash
ls -la /var/www/html/
```
...
```
total XX
drwxr-xr-x. 3 apache apache   XX ... .
drwxr-xr-x. 4 root   root     XX ... ..
-rw-r--r--. 1 apache apache XXXX ... index.php
drwxr-xr-x. 2 apache apache   XX ... images
... (other PHP files)
```

**Step 4.6 - Test PHP execution:**
```bash
curl http://localhost/index.php
```

Expected: HTML output from the PHP application (might show database connection errors at this stage - this is normal since RDS doesn't exist yet)

---

#### Method B: Deploy via GitHub (Faster Alternative)

If your code is on GitHub and your instance has internet access (via NAT Gateway):

**Step B.1 - Install Git:**
```bash
...
#### Method C: Upload via S3 (Alternative)


If Session Manager upload doesn't work:

**Step C.1 - Upload to S3 from your computer:**
```bash
# From your local terminal (not EC2)
aws s3 cp website.zip s3://your-bucket-name/website.zip
```

**Step C.2 - Download to EC2 from Session Manager terminal:**
```bash
aws s3 cp s3://your-bucket-name/website.zip /home/ec2-user/website.zip
sudo unzip /home/ec2-user/website.zip -d /var/www/html/
sudo chown -R apache:apache /var/www/html/
sudo chmod -R 755 /var/www/html/
```
...
**Verification:**
- /var/www/html/ contains index.php and all application files
- Files are owned by apache:apache
- curl http://localhost/index.php returns HTML

📸 **Screenshot Required:** 
- Terminal showing `ls -la /var/www/html/` with PHP files
- Terminal showing `curl http://localhost/index.php` with HTML output
...
- [ ] A3-G7-EC2-AZ1 launched and running
- [ ] Instance is in A3-G7-Private-EC2-Subnet-AZ1
- [ ] Instance has no public IP (correct)
- [ ] IAM role A3-G7-SSM-Role attached
...
1. ✅ A3-G7-EC2-AZ1 instance details
...
Select both:
- ✅ **10.0.5.0/24** (A3-G7-Private-RDS-Subnet-AZ1)
- ✅ **10.0.6.0/24** (A3-G7-Private-RDS-Subnet-AZ2)
...
| Field | Value |
|---|---|
| DB instance identifier | `A3-G7-RDS` |
...
| Field | Value |
|---|---|
| Virtual private cloud (VPC) | Select `A3-G7-VPC` |
| DB subnet group | Select `A3-G7-DB-Subnet-Group` |
| Public access | **No** |
| VPC security group | Choose existing |
| Existing VPC security groups | Remove "default", add `A3-G7-RDS-SG` |
...
3. In the dropdown, search and select `A3-G7-RDS-SG`
...
"Successfully created database A3-G7-RDS"
...
Click on `A3-G7-RDS` to view details
...
| VPC security groups | A3-G7-RDS-SG (active) |
| Subnet group | A3-G7-DB-Subnet-Group |
| Availability Zone | us-east-1a |
...
### TASK 3: Connect to EC2 and Import Database

**Navigation:**
1. AWS Console → Search "Systems Manager" → **Systems Manager**
2. Left sidebar → **Session Manager**
3. Click **Start session**
4. Select **A3-G7-EC2-AZ1**
5. Click **Start session**

Terminal opens. Switch to ec2-user:
```bash
sudo su - ec2-user
```
...
The EC2 instance needs the MySQL client to connect to RDS. In Amazon Linux 2023, this is provided by the MariaDB package.
...
#### Step 3.2 - Upload SQL Dump File to EC2

You have a `database.sql` file containing the on-premises database data.

**Method A - Session Manager Upload:**
1. In Session Manager terminal, click **Upload file** button (top right)
2. Browse to `database.sql`
3. Click **Open**
4. Wait for success message
5. Move file:
```bash
sudo mv /home/ssm-user/database.sql /home/ec2-user/
sudo chown ec2-user:ec2-user /home/ec2-user/database.sql
```

**Method B - S3 Upload (Alternative):**
```bash
# From your local computer
aws s3 cp database.sql s3://your-bucket/database.sql

# From EC2 Session Manager terminal
aws s3 cp s3://your-bucket/database.sql /home/ec2-user/database.sql
```

**Method C - GitHub Download (Faster Alternative):**
If you pushed your `.sql` file to your GitHub repo (e.g., `PHP-Script`):

```bash
cd ~/PHP-Script
git pull
```
The file will now be available at `~/PHP-Script/Countrydatadump.sql`.

**Verify file is present:**
```bash
ls -lh /home/ec2-user/database.sql
```

Expected output showing the file size (should be a few KB).

---

#### Step 3.3 - Test RDS Connection

Replace `<RDS-ENDPOINT>` with your actual endpoint:
...
You should see `countries` in the list. This confirms RDS was created with the initial database name.
...
📸 **Screenshot Required:** Terminal showing successful MySQL connection and SHOW DATABASES output with "countries" listed

---

#### Step 3.4 - Understand the SQL Dump File Structure

**CRITICAL CONTEXT:**

The database.sql file header may say:
```sql
-- MySQL dump 10.13  Distrib 5.7.33, for Linux (x86_64)
--
-- Host: localhost    Database: country_schema
```

⚠️ **IGNORE "country_schema"** - this is old export metadata from the on-premises system.

**The correct database name is "countries"** (as specified in the assignment and Parameter Store).
...
- NO "USE database" statement

This means:
1. The database must already exist (RDS created it with "Initial database name = countries")
2. You must specify which database to import into

---

#### Step 3.5 - Import the SQL Dump into RDS

**Command structure:**
```bash
mysql -h <endpoint> -u admin -p <database-name> < <sql-file>
```

**Actual command:**
```bash
mysql -h <YOUR-RDS-ENDPOINT> -u admin -p countries < ~/PHP-Script/Countrydatadump.sql
```
...
**What happens:**
- MySQL connects to RDS
- Selects the "countries" database
- Executes all SQL statements in the file
- Creates the countrydata_final table
- Inserts 215 rows of data
...
**If you see errors:**
- Check the database name is "countries"
- Check the password is correct
- Verify RDS endpoint is correct
- Ensure database.sql file exists

📸 **Screenshot Required:** Terminal showing the import command with no errors

---

#### Step 3.6 - Verify Data Import

Connect to MySQL:
```bash
mysql -h a3-g7-rds.c1234567890.us-east-1.rds.amazonaws.com -u admin -p
```
...
**Verify data structure and sample rows:**
```sql
...
```
+-------------+------------+---------+----------------+-----------+--------------+------------------+-----------------------------+------------------------------+-----------------+
| name        | population | GDP     | lifeexpectancy | birthrate | mobilephones | mortalityunder5  | healthexpenditurepercapita  | healthexpenditurepercentGDP | populationurban |
+-------------+------------+---------+----------------+-----------+--------------+------------------+-----------------------------+------------------------------+-----------------+
| Afghanistan | 32526562.0 | 19.36   | 60.37          | 38.57     | 13.6         | 97.3             | 57.7                        | 8.19                        | 24              |
| Albania     | 3029278.0  | 11.86   | 77.41          | 12.73     | 98.9         | 14.2             | 247.5                       | 5.88                        | 53              |
...
```
...
Expected output showing all column definitions including:
- name
- population
- GDP
- lifeexpectancy
...
|---|---|---|
| Database "countries" exists | Yes | |
| Table "countrydata_final" exists | Yes | |
| Row count | 215 | |
| All columns present | 10 columns | |
| Sample data displays correctly | Yes | |

📸 **Screenshot Required:**
- SHOW TABLES output
- SELECT COUNT(*) showing data rows
- SELECT * LIMIT 5 showing sample country data
- DESCRIBE output showing all columns

**NEXT STEP:** Now that the database is ready, you must update the Parameter Store so your website can connect to it.

➡️ **Proceed to [TASK 4: Update Parameter Store](#task-4-update-parameter-store-and-notify-member-1)**

---

### TASK 4: Update Parameter Store and Notify Member 1

**Navigation:**
1. AWS Console → **Systems Manager**
2. Left sidebar → **Parameter Store**
3. Click on `/example/endpoint`
4. Click **Edit** (top right)

**Update the parameter:**
| Field | Current Value | New Value |
|---|---|---|
| Value | PLACEHOLDER | `<YOUR-RDS-ENDPOINT>` |

Replace with your actual RDS endpoint: `a3-g7-rds.c0bortbeujiu.us-east-1.rds.amazonaws.com`

⚠️ **CRITICAL:** Paste the endpoint EXACTLY - no spaces, no extra characters, no http://, just the hostname.

Click **Save changes**

**Verification:**
- Parameter value now shows your RDS endpoint instead of "PLACEHOLDER"

📸 **Screenshot Required:** Parameter Store showing /example/endpoint with actual RDS endpoint value

---

#### Step 4.2 - Test your Live Website

Now that the Parameter Store is updated, test your application from the EC2 terminal:

```bash
curl http://localhost/index.php
```

You should see HTML output showing real country data. If you see it, **Member 2's job is complete!**
...
> "Database migration complete. RDS endpoint is:
> a3-g7-rds.c1234567890.us-east-1.rds.amazonaws.com
>
> I have updated /example/endpoint in Parameter Store.
>
> Please test the PHP application to verify database connectivity."

---

**Member 1 should now test the website:**

Member 1 connects to EC2 via Session Manager and runs:
```bash
curl http://localhost/index.php
```

If successful, the output should show actual country data from the database instead of connection errors.

---

## Member 2 - Final Checklist

- [ ] A3-G7-DB-Subnet-Group created with both RDS subnets
- [ ] A3-G7-RDS instance created with Production template
- [ ] Multi-AZ = Yes
- [ ] Publicly accessible = No
- [ ] Security group = A3-G7-RDS-SG
- [ ] Initial database "countries" created
- [ ] MySQL client installed on EC2
- [ ] database.sql imported successfully
- [ ] 215 rows confirmed in countrydata_final table
- [ ] All columns verified (name, population, GDP, etc.)
- [ ] /example/endpoint updated in Parameter Store
- [ ] Member 1 notified with RDS endpoint
- [ ] All required screenshots captured

### Screenshots Checklist for Member 2:
1. ✅ A3-G7-DB-Subnet-Group details
2. ✅ A3-G7-RDS showing Available, Multi-AZ, Not publicly accessible
3. ✅ RDS endpoint URL
4. ✅ MySQL connection successful (SHOW DATABASES)
5. ✅ Import command executed with no errors
6. ✅ SHOW TABLES output
7. ✅ SELECT COUNT(*) showing 215
8. ✅ SELECT * LIMIT 5 showing data
9. ✅ DESCRIBE countrydata_final
10. ✅ Updated /example/endpoint parameter

**Notify Member 4:** "Database migration complete. RDS is ready. You can begin Task 4 (High Availability & Scalability)."
...
### Prerequisites:
- Member 1 has fully configured A3-G7-EC2-AZ1 with PHP application
...
3. Find and select **A3-G7-EC2-AZ1** (checkbox on left)
4. Click **Actions** (top right) → **Image and templates** → **Create image**

**Create image:**
| Field | Value |
|---|---|
| Image name | `A3-G7-AMI` |
...
📸 **Screenshot Required:** AMI list showing A3-G7-AMI with Status = Available
...
|---|---|
| Source | **My AMIs** (tab) |
| Amazon Machine Image (AMI) | Select `A3-G7-AMI` |

You should see: "A3-G7-AMI (ami-xxxxxx)"
...
| Firewall (security groups) | Select existing security group |
| Security groups | Select `A3-G7-EC2-SG` |
...
|---|---|---|
| `Name` | `A3-G7-EC2-AZ2` | ✅ Instances, ✅ Volumes |
...
Success message: "Successfully created A3-G7-LT"

Click **View launch template**

**Verification:**
- Template name: A3-G7-LT
- Latest version: 1
- AMI: A3-G7-AMI (ami-xxxxxx)
- Instance type: t2.small
- Security group: A3-G7-EC2-SG
- IAM role: A3-G7-SSM-Role
- Tags: Name=A3-G7-EC2-AZ2
...
**Availability Zone us-east-1a:**
- ✅ Check us-east-1a
- Subnet: Select `A3-G7-Public-Subnet-AZ1 (10.0.1.0/24)`

**Availability Zone us-east-1b:**
- ✅ Check us-east-1b
- Subnet: Select `A3-G7-Public-Subnet-AZ2 (10.0.2.0/24)`
...
| Select security groups | Select `A3-G7-ALB-SG` |
...
| Protocol | HTTP |
| Port | 80 |
| Default action | Forward to `A3-G7-TG` |

Click on the "Forward to" dropdown and select `A3-G7-TG`
...
- Security group: A3-G7-ALB-SG
- Listener: HTTP:80 → A3-G7-TG
...
- Find "DNS name" field
- It looks like: `A3-G7-ALB-1234567890.us-east-1.elb.amazonaws.com`
- Click the copy icon
...
| Field | Value |
|---|---|
| Auto Scaling group name | `A3-G7-ASG` |
| Launch template | Select `A3-G7-LT` |
...
- ✅ `A3-G7-Private-EC2-Subnet-AZ1 | us-east-1a | 10.0.3.0/24`
- ✅ `A3-G7-Private-EC2-Subnet-AZ2 | us-east-1b | 10.0.4.0/24`
...
- Existing load balancer target groups: Select `A3-G7-TG`
...
| Field | Value |
|---|---|
| Scaling policy name | `A3-G7-CPU-Policy` |
...
Tags are already configured in the Launch Template (Name=A3-G7-EC2-AZ2).
...
**Review all settings:**
- Name: A3-G7-ASG
- Launch template: A3-G7-LT
- VPC: A3-G7-VPC
- Subnets: 2 private EC2 subnets (AZ1 and AZ2)
- Load balancer: A3-G7-TG
...
Go to Auto Scaling Groups → A3-G7-ASG → Activity tab
...
```
http://A3-G7-ALB-1234567890.us-east-1.elb.amazonaws.com/index.php
```
...
1. Select one of the ASG-launched instances (A3-G7-EC2-AZ2)
...
**Check Target Group:**
1. EC2 → Target Groups → A3-G7-TG → Targets tab
...
**Check Auto Scaling Group:**
1. EC2 → Auto Scaling Groups → A3-G7-ASG → Activity tab
...
Reason: Monitor "A3-G7-CPU-Policy" triggered a scale-out activity.
```
...
- [ ] A3-G7-AMI created and available
- [ ] A3-G7-LT created with correct AMI, instance type, security group, IAM role
- [ ] Launch template has Name tag for instances
- [ ] A3-G7-TG created with /index.php health check
- [ ] A3-G7-ALB created, internet-facing, across 2 public subnets
- [ ] ALB state = Active
- [ ] A3-G7-ASG created with min=1, desired=2, max=4
- [ ] ASG uses both private EC2 subnets
- [ ] ASG attached to A3-G7-TG
...
1. ✅ A3-G7-AMI showing Available status
2. ✅ A3-G7-LT details (AMI, instance type, SG, IAM role)
3. ✅ A3-G7-TG health check configuration
4. ✅ A3-G7-ALB showing Active, DNS name, 2 AZs
5. ✅ A3-G7-ASG configuration (capacity, subnets, policy)
...
**Steps:**
1. Open browser
2. Navigate to: `http://A3-G7-ALB-xxxxxx.us-east-1.elb.amazonaws.com/index.php`
3. Verify:
   - ✅ Page loads
...
|---|---|---|
| Public Subnet | ✅ A3-G7-Public-Subnet-AZ1 | ✅ A3-G7-Public-Subnet-AZ2 |
| Private EC2 Subnet | ✅ A3-G7-Private-EC2-Subnet-AZ1 | ✅ A3-G7-Private-EC2-Subnet-AZ2 |
| Private RDS Subnet | ✅ A3-G7-Private-RDS-Subnet-AZ1 | ✅ A3-G7-Private-RDS-Subnet-AZ2 |
...
```
Internet → A3-G7-ALB-SG (port 80 from 0.0.0.0/0)
           ↓
         A3-G7-EC2-SG (port 80 from A3-G7-ALB-SG only)
           ↓
         A3-G7-RDS-SG (port 3306 from A3-G7-EC2-SG only)
```

**IAM Validation:**
- ✅ EC2 instances have A3-G7-SSM-Role
...
| ALB shows no targets | Target Group not attached | Verify ASG is attached to A3-G7-TG |
...
**Decision:** Single NAT Gateway in A3-G7-Public-Subnet-AZ1 only
...
Internet
   ↓
A3-G7-ALB-SG (Allow: HTTP/80 from 0.0.0.0/0)
   ↓
A3-G7-EC2-SG (Allow: HTTP/80 from A3-G7-ALB-SG, SSH/22 from Admin IP)
   ↓
A3-G7-RDS-SG (Allow: MySQL/3306 from A3-G7-EC2-SG)
```
...
#### IAM Role Design: A3-G7-SSM-Role
...
2. Attacker attempts: `aws rds delete-db-instance --db-instance-identifier A3-G7-RDS`
...
- IAM role controls access (A3-G7-SSM-Role)
...
**Workflow:**
```
1. Launch A3-G7-EC2-AZ1
2. Install Apache + PHP
3. Deploy application files
4. Test thoroughly
5. Create A3-G7-AMI (snapshot)
...
- Version AMIs (A3-G7-AMI-v1, v2, v3)
...
- ✅ All resources have systematic names (A3-G7-*)
...
- ✅ Least-privilege IAM role (A3-G7-SSM-Role)
